import React, { useCallback, useEffect, useState } from 'react';
import { Pressable, Text, View } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';

import { router } from 'expo-router';

import { auth } from '@/firebase';
import { loadLocalCvData } from '@/resume/storage';

export default function ResumeScreen() {
  const [lastUpdated, setLastUpdated] = useState<string | null>(null);

  const refresh = useCallback(async () => {
    const cv = await loadLocalCvData(auth.currentUser?.uid);
    setLastUpdated(cv.updatedAt ?? null);
  }, []);

  useEffect(() => {
    refresh();
  }, [refresh]);

  const lastUpdatedText = lastUpdated ? new Date(lastUpdated).toLocaleString() : 'Not saved yet';

  return (
    <SafeAreaView className="flex-1 bg-white" edges={['top', 'left', 'right']}>
      <View className="px-5 pb-4 pt-2">
        <Text className="text-2xl font-semibold">Resume</Text>
      </View>

      <View className="flex-1 px-5">
        <Text className="mb-4 text-sm text-gray-700">CV last updated: {lastUpdatedText}</Text>

        <Pressable
          onPress={() => router.push('/(tabs)/resume-preview?template=ats')}
          className="rounded border border-gray-200 bg-white p-4 active:opacity-80">
          <Text className="text-base font-semibold">ATS Simple (Recommended)</Text>
          <Text className="mt-1 text-sm text-gray-600">
            Single-column, text-first, recruiter/ATS friendly.
          </Text>
        </Pressable>

        <View className="mt-4">
          <Pressable
            onPress={refresh}
            className="rounded border border-gray-300 px-4 py-3 active:opacity-80">
            <Text className="text-center font-semibold">Refresh</Text>
          </Pressable>
        </View>
      </View>
    </SafeAreaView>
  );
}
